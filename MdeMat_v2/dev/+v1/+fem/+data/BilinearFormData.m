classdef BilinearFormData < handle
    % BILINEARFORMDATA A collection of data for a bilinear form.
    %
    %   BilinearFormData represents a bilinear form through its matrix
    ...
    %   form = BilinearFormData();
    %   
    %   % Create mass matrix (0,0 derivatives)
    %   massForm = BilinearFormData(functionData1, functionData2, 0, 0);
    %   M = massForm.matrix;
    %
    %   % Create stiffness matrix (1,1 derivatives) 
    %   stiffForm = BilinearFormData(functionData1, functionData2, 1, 1);
    %   K = stiffForm.matrix;
    %
    %   % Mixed derivative form
    %   mixedForm = BilinearFormData(functionData1, functionData2, 0, 1);
    %
    % See also:
    %   approx.fem.data.FunctionData, approx.integrate.Integrator
    ...
    methods
        function obj = BilinearFormData(D1, D2, i, j)
            % BILINEARFORMDATA Constructor for BilinearFormData.
            %
            %   obj = BilinearFormData() creates an empty bilinear form
            %   data object with uninitialized matrix and vector
            %   properties.
            %
            %   obj = BilinearFormData(D1, D2, i, j) creates a bilinear
            %   form data object and initializes the matrix representation
            %   from function data using the specified derivative orders.
            %
            % Inputs:
            %   D1 - First function data object (FunctionData)
            %   D2 - Second function data object (FunctionData)
            ...
            %
            % Outputs:
            %   obj - Constructed BilinearFormData object
            
            if nargin == 0
                obj.matrix = []; 
                obj.vector = [];
            else
                obj.setMatrix(D1, D2, i, j);
            end
        end

        function setMatrix(obj, D1, D2, i, j)
            % SETMATRIX Set the matrix representation of the bilinear form.
            %
            %   setMatrix(obj, D1, D2, i, j) computes the matrix
            %   representation as the weighted inner product between
            %   function values or derivatives from two function data
            %   objects. The computation follows the discrete bilinear
            %   form:
            %
            %   \f[
            ...
            %
            % Inputs:
            %   obj - The BilinearFormData object
            %   D1 - First function data object (FunctionData)
            %   D2 - Second function data object (FunctionData)
            %   i - Derivative order for first function (non-negative integer, 0 for values)
            %   j - Derivative order for second function (non-negative integer, 0 for values)
