classdef L2SemiLagrangianFiniteElementOperator < fem.element.C0SemiLagrangianFiniteElementOperator
    ...
    methods
        ...
        function setFluxPieceData(obj)
            % SETFLUXPIECEDATA Set up bilinear form data for flux piece integrals.
            %
            %   setFluxPieceData(obj) creates bilinear form matrices for
            %   each flux piece resulting from the intersection of element
            %   boundaries with upstream footprints. These matrices enable
            %   accurate computation of flux terms in semi-Lagrangian
            ...
            for i = 1:nf
                ...
                obj.fluxPieceData{i} = arrayfun( ...
                    @(k, l) fem.data.BilinearFormData(De{i}(k, l), Du{i}(k, l), 0, 0), ...
                    repmat((1:2).', 1, np(i)), repmat(1:np(i), 2, 1));
            end
        end

        function G = buildGradient(obj)
            % BUILDGRADIENT Build gradient operator with flux piece contributions.
            %
