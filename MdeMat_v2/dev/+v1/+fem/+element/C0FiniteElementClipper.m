classdef C0FiniteElementClipper < fem.element.FiniteElementClipper
    % C0FINITEELEMENTCLIPPER Geometric clipper for continuous finite elements.
    %
    ...
    methods
        function setVolumePieceData(obj, ht, hx)
            % SETVOLUMEPIECEDATA Set up volume piece data for semi-Lagrangian transport.
            %
            %   setVolumePieceData(obj, ht, hx) computes the geometric
            ...
            obj.volumeEulerianData = arrayfun(@(i) fem.data.FunctionData(I.copy()), 1:np);
            obj.volumeUpstreamData = arrayfun(@(i) fem.data.FunctionData(I.copy()), 1:np);
