classdef C0SemiLagrangianFiniteElementOperator < fem.element.SemiLagrangianFiniteElementOperator
   ...
   properties (Dependent)
    ...
    methods
        ...
        function setVolumePieceData(obj)
            % SETVOLUMEPIECEDATA Set up bilinear form data for volume piece integrals.
            %
            %   setVolumePieceData(obj) creates bilinear form matrices for
            %   each volume piece resulting from the intersection of
            %   Eulerian and upstream elements. These matrices enable
            %   accurate integration over the complex geometries arising
            %   in semi-Lagrangian methods.
            %
            % Inputs:
            %   obj - The C0SemiLagrangianFiniteElementOperator object
            %
            ...
            obj.volumePieceData = arrayfun( ...
                @(i, j) fem.data.BilinearFormData(De(j), Du(j), i, 0), ...
