classdef L2FiniteElementClipper < fem.element.C0FiniteElementClipper
    % L2FINITEELEMENTCLIPPER Geometric clipper for discontinuous finite elements.
    %
    %   L2FiniteElementClipper extends the C0 clipper with flux piece
    %   computations for discontinuous finite elements in semi-Lagrangian
    %   methods. It handles both volume intersections (inherited from C0)
    %   and boundary flux intersections required for discontinuous Galerkin
    %   transport schemes.
    %
    ...
    %
    %   This approach enables semi-Lagrangian discontinuous Galerkin methods
    %   that combine the stability benefits of characteristic tracking with
    %   the flexibility of discontinuous function spaces, particularly
    %   effective for problems with discontinuous solutions or strong
    %   convection.
    %
    % Examples:
    %   % Create clipper for L2 element with specified velocity
    %   velocity = [0.5; 0.3];
    %   element = fem.element.L2FiniteElement(geometry, projector, volInt, fluxInt);
    %   clipper = L2FiniteElementClipper(element, velocity);
    %   
    ...
    %
    % See also:
    %   fem.element.C0FiniteElementClipper, fem.element.L2FiniteElement,
    ...
    properties
        ...
        fluxPieces % Geometric data for flux pieces (structure array)
                   % Array indexed by boundary face, each containing:
                   % .xc - Center coordinates of pieces
                   % .hx - Dimensions of pieces (zero in normal direction)
    ...
    methods
        function setFluxPieceData(obj, ht, hx)
            % SETFLUXPIECEDATA Set up flux piece data for semi-Lagrangian transport.
            %
            %   setFluxPieceData(obj, ht, hx) computes the geometric
            ...
            %
            % Inputs:
            %   obj - The L2FiniteElementClipper object
            ...
            I = obj.fe.fluxData.integrator;
            obj.fluxEulerianData = cell(1, nf);
            obj.fluxUpstreamData = cell(1, nf);
            for i = 1:nf
                obj.fluxEulerianData{i} = arrayfun(@(j, k) fem.data.FunctionData(I.copy()), ...
                    repmat((1:2).', 1, np(i)), repmat(1:np(i), 2, 1));
                obj.fluxUpstreamData{i} = arrayfun(@(j, k) fem.data.FunctionData(I.copy()), ...
                    repmat((1:2).', 1, np(i)), repmat(1:np(i), 2, 1));
            end
            ...
            for i = 1:nf
                for j = 1:np(i)
                    ...
                    obj.fluxEulerianData{i}(1, j).setPoints(nq, false, a, b);
                    obj.fluxEulerianData{i}(2, j).setPoints(nq, false, a, b);
                    obj.fluxUpstreamData{i}(1, j).setPoints(nq, false, -b, -a);
                    obj.fluxUpstreamData{i}(2, j).setPoints(nq, false, -b, -a);
                end
            end
            ...
            obj.fluxShifts = arrayfun(@(i) zeros(nd, np(i)), 1:nf, 'UniformOutput', false);
            for i = 1:nf
