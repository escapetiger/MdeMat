classdef L2FiniteElementOperator < fem.element.C0FiniteElementOperator
    ...
    methods
        ...
        function setFluxData(obj)
            % SETFLUXDATA Set up bilinear form data for flux integrals.
            %
            %   setFluxData(obj) creates a tensor of bilinear form matrices
            %   for flux computations, organizing data by derivative order,
            %   flux boundary, and flow direction. This structure enables
            %   efficient assembly of DG operators with proper treatment
            %   of inflow and outflow contributions.
            %
            % Inputs:
            ...
            obj.fluxData = arrayfun(@(i) fem.data.BilinearFormData(), 1:nd*nr*nf);
            ...
            for i = 1:nf
                ...
                for j = 0:nr
                    ...
                    obj.fluxData(1, j+1, i).setMatrix(D(i), D(i), j, 0);
                    %< Outflow contribution (neighboring element)
                    obj.fluxData(2, j+1, i).setMatrix(D(i), D(k), j, 0);
                end
            end
        end
    end
