...
config.ic.f = fInit(config.E, config.omega, config.epsilon); % Initial condition
config.bc.f = []; % Boundary condition
config.exacts.f = fExact(config.E, config.omega, config.lambda, config.epsilon); % Exact solution
% config.exacts = [];
