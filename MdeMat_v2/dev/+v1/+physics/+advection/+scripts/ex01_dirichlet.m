...
config.schemeId = sprintf('%s%s%d', upper(config.tDiscId), upper(config.xDiscId), config.xBasisOrder); % Scheme ID
config.L = 1; % Length
...
config.bc = uExact(config.advection, config.omega); % Boundary condition
config.exacts.U = uExact(config.advection, config.omega); % Exact solution
% config.exacts = [];
