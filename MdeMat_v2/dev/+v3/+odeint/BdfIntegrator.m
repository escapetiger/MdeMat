                ...
                if nargin(M) == 1
                    for j = 1:m
                        if isempty(obj.MRhs) || obj.timeline.hasStepSizeChanged 
                            obj.MRhs{j} = M(t0 + dt - sum(obj.timeline.h(1:j)));
                        end
                    end
                    Me = obj.MRhs;
                elseif nargin(M) == 2
                    Me = cell(m, 2);
                    for j = 1:m
                        if isempty(obj.MRhs{j}) || obj.timeline.hasStepSizeChanged                
                            [Me{j, 1:2}] = M(t0 + dt, t0 + dt - sum(obj.timeline.h(1:j)));
                            obj.MRhs{j} = Me{j, 2};
                        else
                            Me{j, 2} = obj.MRhs{j};
                            Me{j, 1} = M(t0 + dt, t0 + dt - sum(obj.timeline.h(1:j)));
                        end
                    end
                end
            else
