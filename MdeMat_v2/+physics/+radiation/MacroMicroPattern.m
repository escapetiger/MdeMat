classdef MacroMicroPattern < handle
    ...
    methods
        ...
        function obj = setDecomposition(obj, state)
            d = state.nDims;
            m = state.nMacroModes;
            n = state.nMicroModes;
        ...
        function obj = setFreeStreaming(obj, state)
            TOL = 1e-13;
            m = state.nMacroModes;
            n = state.nMicroModes;
            d = state.nDims;
            ...
            B = U ./ diag(U.'*W*U).';
            if m > 0 && n == 0
                obj.streaming = zeros(m, m, d);
                for i = 1:d
                    C = diag(V(i, :));
                    obj.streaming(:, :, i) = U.' * W * C * B;
                end
            elseif m == 0 && n > 0
                obj.streaming = zeros(n, n, d);
                for i = 1:d
                    C = diag(V(i, :));
                    obj.streaming(:, :, i) = (I - B * U.' * W) * C;
                end
            else
