%==========================================================================
% FileName: ex07_two_peaks
% Author: Yi CAI
% Description:
%   Two-peaked Gaussian initial distribution test for radiation transport.
%   Two isotropic Gaussian density peaks propagate and interact through a
...
config.ckptTimeStamps = [0.05, 0.1, 0.2, 0.5, 1.0];
config.verbose = 2;
config.tFinal = 1.0;
...
for k = 1:length(epsilon)
    ...
    switch config.id
        case 'ex07_2dt'
            config.ckptPrefix = strjoin({'TP2DT', config.ckptPrefix}, '_');
    end
...
E = config.E;
sigma = sqrt(1e-2);
x1 = [-0.3; zeros(size(x, 1) - 1, 1)];
x2 = [0.3; zeros(size(x, 1) - 1, 1)];
R1 = sqrt(sum((x - x1).^2, 1));
R2 = sqrt(sum((x - x2).^2, 1));
f = (exp(-R1.^2 / (2 * sigma^2)) + exp(-R2.^2 / (2 * sigma^2))) * E;
end
