classdef FiniteElementUpwindAssembly < approx.assembly.FiniteElementAssembly
    % FINITEELEMENTUPWINDASSEMBLY Assembly for finite element upwind
    % operators.
    %
    %   FiniteElementUpwindAssembly handles the assembly of upwind finite
    ...
    methods
        function obj = FiniteElementUpwindAssembly(space, bcType)
            % FINITEELEMENTUPWINDASSEMBLY Constructor for upwind assembly.
            %
            %   obj = FiniteElementUpwindAssembly(space, bcType) creates an
        ...
        function obj = setBcType(obj, bcType)
            ...
            arguments
                obj approx.assembly.FiniteElementUpwindAssembly
                bcType{mustBeMember(bcType, {'periodic', 'dirichlet'})}
            end
        ...
        function G = assembleVector(obj, field, bias, func, options)
            ...
            G = obj.assembleFluxVector(field, bias, func, options.args{:});
        end
    end
    ...
    methods (Access = private)
        ...
        function G = assembleFluxVector(obj, field, bias, func, varargin)
            ...
            for LFI = 1:obj.Space.Element.NFluxes
                ...
                EI = obj.Space.Mesh.findBoundaryElements(BEI, LFI);
                if ~isempty(EI)
                    GF = obj.assembleBoundaryFluxVector(field, bias, func, EI, LFI, varargin{:});
                    G = G + GF;
                end
            end
        end
        
        function G = assembleBoundaryFluxVector(obj, field, bias, func, EI, LFI, varargin)
