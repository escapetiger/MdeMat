classdef FiniteElementAdjointAssembly < approx.assembly.FiniteElementAssembly
    % FINITEELEMENTADJOINTASSEMBLY Assembly for finite element adjoint
    % operator pairs.
    %
    %   FiniteElementAdjointAssembly implements assembly operations for
    ...
    %   approx.assembly.FiniteElementEllipticAssembly
    ...
    methods
        function obj = FiniteElementAdjointAssembly(space, bcType, penaltyType)
        ...
        function obj = setBcType(obj, bcType)
            ...
            arguments
                obj approx.assembly.FiniteElementAdjointAssembly
                bcType{mustBeMember(bcType, {'periodic', 'dirichlet'})}
            end
        ...
        function G = assembleVector(obj, field, func, options)
            ...
            if isa(field, 'function_handle')
                GF = obj.assemblePointwiseFluxVector(field, func, options.args{:});
            else
    ...
    methods (Access = protected)
        ...
        function G = assembleConstantFluxVector(obj, field, func, varargin)
            ...
            for LFI = 1:obj.Space.Element.NFluxes
                ...
                EI = obj.Space.Mesh.findBoundaryElements(EIB, LFI);
                if ~isempty(EI)
                    GF = obj.assembleConstantBoundaryFluxVector(field, func, EI, LFI, varargin{:});
                    for d = 1:nd
                        G{1, 1}{d} = G{1, 1}{d} + GF{1, 1}{d};
                        G{2, 1}{d} = G{2, 1}{d} + GF{2, 1}{d};
                    end
        ...
        function G = assembleConstantBoundaryFluxVector(obj, field, func, EI, LFI, varargin)
            ...
            U = obj.Space.feval(func, xRef, EI=EI, args = varargin); % (nd, nq, ne)
