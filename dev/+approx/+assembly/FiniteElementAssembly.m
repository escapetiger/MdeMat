classdef FiniteElementAssembly < approx.assembly.Assembly
    ...
    %
    % See also:
    %   approx.assembly.Assembly, approx.space.FiniteElementSpace
    ...
    methods
        function obj = FiniteElementAssembly(space)
            ...
            obj@approx.assembly.Assembly(space);
        end
    end
    ...
    methods (Access = protected)
        ...
        function G = createVectorBatch(obj, L, options)
            % CREATEVECTORBATCH Create global sparse matrix from a batch of
            % local vectors.
            %
            %   G = createVectorBatch(obj, L) assembles batched local vectors
            %   @a L to a global sparse matrix @a G using all elements and DoFs.
            %
            %   G = createVectorBatch(obj, L, TEI=TEI, TLI=TLI) assembles with
