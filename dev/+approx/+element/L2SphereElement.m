classdef L2SphereElement < approx.element.L2Element
    ...
    methods (Static)
        function obj = modal(nDims, order, options)
            ...
            %   orthogonal basis functions (Legendre, Fourier, or Spherical Harmonics).
            %
            ...
            if isempty(options.np)
                ...
                elseif nDims == 2 && strcmp(reduction, 'topology')
                    np = max(16, ceil((3 * order + 2)/4)+1);
                else
                    np = max([4, 4], repmat(order+2, 1, 2));
                end
            else
                np = options.np;
            end
            ...
            if nDims == 1 && strcmp(reduction, 'topology')
                ...
                maxDegree = order - 1;
                F = core.function.LegendreBasis(maxDegree, isNormalized = false);
            ...
            elseif nDims == 2 && strcmp(reduction, 'topology')
                ...
                maxDegree = order - 1;
                F = core.function.FourierBasis(maxDegree, isNormalized = false);
                ...
                maxDegree = order - 1;
                F = core.function.SphericalHarmonicBasis(maxDegree, isNormalized = false);
            end

            %< Create basis function
            B = core.function.Product(F, E);
