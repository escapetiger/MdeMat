...
config.ckptDir = fullfile(fileparts(mfilename('fullpath')), 'ex03h');
...
config.tFinal = 50;
config.ckptTimeStamps = [20, 30, 40];
...
p = (cos(2*kappa*x) + cos(3*kappa*x)) / (6 / 5) + cos(kappa*x);
D0 = (1 + delta * p);
D2 = 5 / 6 * sqrt(2) * (1 + delta * p);
if nh == 1
    D = D0;
elseif nh == 2
