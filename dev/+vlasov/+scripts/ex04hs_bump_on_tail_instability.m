%==========================================================================
% FileName: ex04hs_bump_on_tail_instability
% Author: Yi CAI
% Date: 13/09/2025
% Description:
%   Strong bump-on-tail instability with Hermite spectral method.
%==========================================================================
...
config.ckptDir = fullfile(fileparts(mfilename('fullpath')), 'ex04hs');
config.ckptPrefix = [upper(config.schemeName), num2str(config.xBasisOrder)];
if config.useFilter
    config.ckptPrefix = ['F', config.ckptPrefix];
end
config.ckptPrefix = strjoin({'BOTHS', config.ckptPrefix}, '_');
config.tFinal = 60;
config.ckptTimeStamps = [12.5, 25, 50];
...
for k = 1:length(nh)
    ...
    % fileName = sprintf('%s_state_%s_t50p0.mat', config.ckptPrefix, config.ckptPostfix);
    ...
    % fileName = sprintf('%s_f_%s_t12p5.pdf', config.ckptPrefix, config.ckptPostfix);
    % fileName = sprintf('%s_f_%s_t25p0.pdf', config.ckptPrefix, config.ckptPostfix);
    % fileName = sprintf('%s_f_%s_t50p0.pdf', config.ckptPrefix, config.ckptPostfix);
    fileName = fullfile(config.ckptDir, fileName);
...
config.T0 = (1.4)^2; % Temperature parameter
config.delta = 0.04; % Perturbation
config.nbot = 3;
config.np = 0.9;
config.nb = 0.1;
config.vp = sqrt(2);
...
% herm = core.function.HermiteBasis(nh-1, isNormalized=true);
% scaling = core.function.Linear(1/sqrt(T0));
% herm = core.function.Composition(herm, scaling);
% fp = @(v) exp(-v.^2/vp.^2) ./ sqrt(pi) ./ vp .* herm.eval(v);
% fb = @(v) exp(-(v-vd).^2/vp.^2) ./ sqrt(pi) ./ vb .* herm.eval(v);
% Dp = integral(fp, -inf, inf, 'ArrayValued', true);
% Db = integral(fb, -inf, inf, 'ArrayValued', true);

Dp = physics.vlasov.expherm(nh-1, 0, vp/sqrt(2*T0));
Db = physics.vlasov.expherm(nh-1, vd/sqrt(T0), vb/sqrt(2*T0));
D = perb(:) .* (np * Dp + nb * Db);
end
