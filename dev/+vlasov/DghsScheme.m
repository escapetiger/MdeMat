classdef DghsScheme < physics.vlasov.VlasovScheme
    ...
    methods
        ...
        function state = step(obj, state)
            ...
            %   step using the DG Hermite spectral method with operator
            %   splitting.
    ...
    methods (Access = private)
        ...
        function state = stepNonlinearImp(obj, state, dt)
            ...
            k = 2:nh;
            DNew(:, k) = D(:, k);
            DNew(:, k) = DNew(:, k) + dt * sqrt(k-1) / sqrt(T0) .* (T * (D(:, k-1) + DNew(:, k-1)) / 2);
            state.Dofs.D = DNew;
        end
        ...
        function state = stepNonlinearSdirk2(obj, state, dt)
            ...
            k = 2:nh;
            D1(:, k) = D(:, k); 
            D1(:, k) = D1(:, k) + a11 * dt * sqrt(k-1) / sqrt(T0) .* (T * D1(:, k-1));
            %< Second stage
            ...
            k = 2:nh;
            D2(:, k) = D(:, k); 
            D2(:, k) = D2(:, k) + a21 * dt * sqrt(k-1) / sqrt(T0) .* (T * D1(:, k-1));
            D2(:, k) = D2(:, k) + a22 * dt * sqrt(k-1) / sqrt(T0) .* (T * D2(:, k-1));
            %< Final stage
            b1 = 1-gamma; b2 = gamma;
            DNew = zeros(size(D));
            DNew(:, 1) = D(:, 1);
            k = 2:nh;
            DNew(:, k) = D(:, k);
            DNew(:, k) = DNew(:, k) + b1 * dt * sqrt(k-1) / sqrt(T0) .* (T * D1(:, k-1));
            DNew(:, k) = DNew(:, k) + b2 * dt * sqrt(k-1) / sqrt(T0) .* (T * D2(:, k-1));
            state.Dofs.D = DNew;
        end
