...
for j = 1:numel(c)
    fprintf('Step source test with c=%.1f\n',c(j));
    for i = 1:numel(x)
        fprintf('=> draw at x=%.4f\n',x(i));
        g = @(k) fn_phi0(k,r,n,c(j),fs);
        phi0(j,i) = cifft(g,x(i),10,0.45)+fn_X(0,r,n,c(j),fs)/2;
    end
end

%% Visulization
figure(1)
plot(x(2:end),phi0(1,2:end),'-', ...
    'LineWidth',1,'DisplayName',sprintf('c=%.1f',c(1))); 
hold on;
for j = 2:numel(c)
    plot(x(2:end),phi0(j,2:end),'--', ...
...
save('data_og1d_plane_step_source_vc.mat');

%% Utils
function X = fn_X(k,r,n,c,fs)
